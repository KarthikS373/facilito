...
// VIEWS
import AnswerTracking from 'views/answerTracking'
...
// PAGE
const FormTrackingPage: NextPage<FormPageProps> = (props) => {
	// COLORES POR DEFECTO
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
	// COOKIE Y PARAMS
	const { formURL, formID, index } = ctx.query
	...
	if (formData && companyID) {
		...
		return {
			props: { formData, company, answerIndex: index, companyURL: formID, formURL },
		}
	} else
		return {
			props: { formData: null, company: null, answer: -1, companyURL: formID, formURL },
		}
}

export default withAuth(FormTrackingPage, { tryAnonymous: true })
